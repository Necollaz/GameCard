    ...
    public class DeckHandCardsPlayer : MonoBehaviour
    {
        ...
        public void AddCard(CardView card)
        {
            ...
            if (card.TryGetComponent(out CardHoverEffect hoverEffect))
                hoverEffect.ForceExitHoverImmediate();
        ...
        private void ArrangeCards()
        {
            ...
            if (count > 0)
            {
                ...
                for (int i = 0; i < cardArrangement.Count; i++)
                {
                    ...
                    card.transform.DOKill(true);
                    _animator.MoveCard(card.transform, pos, _moveDuration).OnComplete(() => { card.transform.localPosition = pos; });
                    _animator.RotateCard(card.transform, targetRotation.eulerAngles, _moveDuration).OnComplete(() => { card.transform.localEulerAngles = targetRotation.eulerAngles; });
            ...
            DOVirtual.DelayedCall(_moveDuration, () =>
            {
                foreach (CardView card in _handCards)
                {
                    if (card.TryGetComponent(out CardHoverEffect hoverEffect) && !hoverEffect.IsHovering)
